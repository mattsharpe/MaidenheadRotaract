...
FixedColumns = function ( oDT, oInit ) {
	...
	this.s = {
		 ...
		 *  @readonly
		 */
		"bOldIE": false//($j.browser.msie && ($j.browser.version == "6.0" || $j.browser.version == "7.0"))
	};
...
FixedColumns.prototype = {
	...
	"_fnConstruct": function ( oInit )
	{
		...
		this.s = $j.extend( true, this.s, FixedColumns.defaults, oInit );
	...
	"_fnCopyLayout": function ( aoOriginal, aiColumns )
	{
		...
		for ( var i=0, iLen=aoOriginal.length ; i<iLen ; i++ )
		{
			...
			{
				if ( $j.inArray( j, aiColumns ) === -1 )
				{
					continue;
				}

				var iCloned = $j.inArray( aoOriginal[i][j].cell, aCloned );
				if ( iCloned === -1 )
				{
